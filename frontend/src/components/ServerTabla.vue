<template>
    <tr>
        <td style="text-align:center; padding: 1px;"> <!--  TIPO SERVER  -->
            ...
            <q-btn no-caps class="tablaAppserverProductivo" v-else-if="server.tiposervidor_id==2 && !server.estado" disable> AppServer </q-btn>
            ...
            <q-btn no-caps class="tablaCromoProductivo" v-else-if="server.tiposervidor_id==3 && !server.estado" disable> Cromo </q-btn>
            ...
            <q-btn no-caps class="tablaDatabaseProductivo" v-else-if="server.tiposervidor_id==4 && !server.estado" disable> Database </q-btn>
        </td>    
        <td> <!--  ROL  -->
            <span> {{server.rolservidor}} </span>
        </td>           
           
        <td style="text-align:center;"> <!-- ESTADO  -->
            <q-icon v-if="server.estado" name="check_circle" size="25px" color="positive" />
            <q-icon v-else name="cancel" size="25px" color="negative" />
        </td>
        ...
        <td> <!-- DISCO  -->  
            <q-linear-progress v-if="parsePorcentaje(server.serverdisco.slice(0, -1))<0.33" rounded size="20px" color="positive" :value="parsePorcentaje(server.serverdisco.slice(0, -1))" stripe padding>
                <div class="absolute-full flex flex-center text-subtitle1">
                <q-badge color="transparent" text-color="black" :label="server.serverdisco" />
                </div>
            </q-linear-progress>
            <q-linear-progress v-if="0.33<parsePorcentaje(server.serverdisco.slice(0, -1)) && parsePorcentaje(server.serverdisco.slice(0, -1))<0.66" rounded size="20px" color="warning" :value="parsePorcentaje(server.serverdisco.slice(0, -1))" stripe>
                <div class="absolute-full flex flex-center text-subtitle1">
                <q-badge color="transparent" text-color="black" :label="server.serverdisco" />
                </div>
            </q-linear-progress>
                <q-linear-progress v-if="parsePorcentaje(server.serverdisco.slice(0, -1))>0.66" rounded size="20px" color="negative" :value="parsePorcentaje(server.serverdisco.slice(0, -1))" stripe>
                <div class="absolute-full flex flex-center text-subtitle1">
    ...
    <q-dialog v-model="dialogo_switch_componentes">
        <q-card class="dialogo_componentes">
            ...
            <q-card-section class="text-white row items-center tarjetaCabeceraDialogo">
                <div class="text-h5">{{server.serverhostname}}</div>
                <q-space />
                ...
                <q-item-label  class="label" header>Componentes de appserver</q-item-label>
                    ...
                    <q-item class="col-12">
                        <div class="row items-center" style="width: 100%;"> 
                            ...
                            <div class="col-2">TCA </div>                    
                            <q-space/>
                            <div class="col-2 text-right" >
                                <q-btn unelevated class="q-py-xs q-mr-xs"  v-if="server.estadotca === 'SI'" icon="power_settings_new" color="primary" size="11px" @click="confirmarDialogo('detener TCA','switchtca')" style="width: 30px;"/>
                                <q-btn outline class="q-py-xs q-mr-xs"  v-if="server.estadotca === 'NO'" icon="power_settings_new" color="primary" size="11px" @click="confirmarDialogo('iniciar TCA','switchtca')" style="width: 30px;"/>
                    ...
                    <q-item class="col-12">
                        <div class="row items-center q-mb-md" style="width: 100%;"> 
                            ...
                            <div class="col-6">OMS Container </div>                    
                            <q-space/>
                            <div class="col-2 text-right" >
        ...
        <div style="overflow: hidden">
            ...
            <div v-if="server.tiposervidor_id==2">
                ...
                <q-card class="tarjetas shadow-5" v-if="server.estado==true">
                    ...
                    <div class="row">
                        ...
                        <div>
                            <q-list dense bordered padding style="height: 145px; border-right: none; border-bottom: none;">
                        ...
                        <div class="col-6">
                        <div>
                            <q-list dense bordered padding style="height: 350px; border-bottom: none;">
                        ...
                        <q-list dense bordered class="col-6 q-pt-sm q-pb-md" style="border-right: none;">
                            <q-item-label  class="label" header>Control Appserver</q-item-label>
                                <div class="q-pl-md q-pr-lg q-gutter-x-lg row">
                                    <div class="col-2">
                                        <q-btn v-if="server.appserverestado === 'DOWN' && canExecute==true && botones_control==true" @click="confirmarDialogo('iniciar el Appserver?','iniciaappserver')"  class="botonAccionesSrv" padding="xs sm" outline icon="login"/>
                                            <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs sm" outline icon="login"/>
                                        <q-tooltip v-if="canExecute==true" class="tooltipAppserver"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"><div class="tooltipText_General"> Iniciar Appserver </div></q-tooltip>
                                    ...
                                    <div class="col-2">
                                        <q-btn v-if="server.appserverestado === 'UP' && canExecute==true && botones_control==true" @click="confirmarDialogo('apagar el Appserver?','apagaappserver')"  class="botonAccionesSrv" padding="xs sm" outline icon="power_settings_new"/>
                                            <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs sm" outline icon="power_settings_new"/>
                                        <q-tooltip v-if="canExecute==true" class="tooltipAppserver"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Detener Appserver </div></q-tooltip>   
                                        <q-tooltip v-else class="bg-negative"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Solo administradores </div> </q-tooltip> 
                                    </div>
                                    <div class="col-2">
                                    <q-btn v-if="server.appserverestado === 'UP' && canExecute==true && botones_control==true" @click="confirmarDialogo('reiniciar el Appserver?','reiniciaappserver')"  class="botonAccionesSrv" padding="xs sm" outline icon="autorenew" />
                                        <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs sm" outline icon="autorenew" />
                                        <q-tooltip v-if="canExecute==true" class="tooltipAppserver"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"><div class="tooltipText_General"> Reiniciar Appserver </div></q-tooltip>
                                        <q-tooltip v-else class="bg-negative"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Solo administradores </div> </q-tooltip>    
                                    </div>
                                </div>
                        </q-list>
                        <q-list dense bordered class="col-6 q-pt-sm q-pb-md">
                            <q-item-label  class="label" header>Componentes</q-item-label>
                                <div class="q-px-md q-mx-sx q-gutter-x-sx text-center self-center">
                                    <q-btn v-if="server.appserverestado === 'UP' && canExecute==true && botones_control==true" @click="dialogo_switch_componentes = true" class="botonAccionesSrv" padding="xs md" outline icon="menu" style="width: 100%;"/>
                                        <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs md" outline icon="menu" style="width: 100%;"/>
                                        <q-tooltip v-if="canExecute==true" class="tooltipAppserver"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Control de componentes </div> </q-tooltip>    
                                        <q-tooltip v-else class="bg-negative"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Solo administradores </div> </q-tooltip>    
                                </div>
                        </q-list>
                    </div>
            ...
            <div v-else-if="server.tiposervidor_id==3">
                ...
                <q-card class="tarjetas shadow-5" v-if="server.estado==true">
                    ...
                    <div class="row">
                        ...
                        <div>
                            <q-list dense bordered padding style="height: 205px; border-bottom: none; width: 337px;">     
                                <q-item-label  class="label" header>Servidor</q-item-label>
                        ...
                        <div>
                            <q-list dense bordered padding style="height: 145px; width: 337px; border-bottom: none;">
                    ...
                    <div class="row">
                        <q-list dense bordered class="q-pt-sm q-pb-md" style="width: 337px;">
                            <q-item-label  class="label" header>Control CROMO</q-item-label>
                                <div class="q-gutter-x-lg row" style="max-width: 150px; position: relative; left: 100px;">
                                    <div class="col">
                                        <q-btn v-if="server.cromoalive === 'NO' && canExecute==true && botones_control==true " @click="confirmarDialogo('iniciar Cromo?','iniciacromo')"  class="botonAccionesSrv" padding="xs sm" outline icon="login"/>
                                        <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs sm" outline icon="login"/>
                                        <q-tooltip v-if="canExecute==true" class="tooltipCromo"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"><div class="tooltipText_General"> Iniciar Cromo </div></q-tooltip>
                                        <q-tooltip v-else class="bg-negative"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Solo administradores </div> </q-tooltip> 
                                    </div>
                                    <div class="col">
                                        <q-btn v-if="server.cromoalive === 'SI' && canExecute==true && botones_control==true" @click="confirmarDialogo('detener Cromo?','apagacromo')"  class="botonAccionesSrv" padding="xs sm" outline icon="power_settings_new"/>
                                        <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs sm" outline icon="power_settings_new"/>
                                        <q-tooltip v-if="canExecute==true" class="tooltipCromo"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Detener Cromo </div></q-tooltip>   
                                        <q-tooltip v-else class="bg-negative"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Solo administradores </div> </q-tooltip> 
                                    </div>
                                    <div class="col">
                                        <q-btn v-if="server.cromoalive === 'SI' && canExecute==true && botones_control==true" @click="confirmarDialogo('reiniciar Cromo?','reiniciacromo')"  class="botonAccionesSrv" padding="xs sm" outline icon="autorenew" />
                                        <q-btn v-else disable color="grey-6" class="botonAccionesSrv" padding="xs sm" outline icon="autorenew" />
                                        <q-tooltip v-if="canExecute==true" class="tooltipCromo"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"><div class="tooltipText_General"> Reiniciar Cromo </div></q-tooltip>
                                        <q-tooltip v-else class="bg-negative"  transition-show="scale" transition-hide="scale" :offset="[0, 5]"> <div class="tooltipText_General"> Solo administradores </div> </q-tooltip>    
                                    </div>
                                </div>
                        </q-list>
            ...
            <div v-else-if="server.tiposervidor_id==1">
                ...
                <q-card class="tarjetas shadow-5" v-if="server.estado==true">
                    ...
                    <q-card-section class="text-white tarjetaCabeceraOnlineGenerico">
                    <div class="text-h5 text-center">{{server.serverhostname.split('.')[0]}}</div>
                    <div class="text-subtitle1 text-center">{{server.serverip}}</div>
                    </q-card-section>
                    ...
                    <div class="row">
                        ...
                        <div>
                            <q-list dense bordered padding style="height: 435px; width: 337px;">     
                                <q-item-label  class="label" header>Servidor</q-item-label>
...
export default {
    ...
    setup(props){
        ...
        var botones_control = ref(true); //desactiva los botones hasta que se ejecute la accion
        var password = ref('');  
        var comandoPost = ref('');  
        
        socket.on('disableControles', (server) => {
            if(props.server.serverip==server){
                botones_control.value=false;
            }
        });

        socket.on('enableControles', (server) => {
            if(props.server.serverip==server){
                botones_control.value=true;
            }
        });        
        ...
        function enviarMensajePost(){
            ...
            if(
             comandoPost.value == 'iniciacromo' || 
             comandoPost.value == 'reiniciacromo' || 
             comandoPost.value == 'apagacromo' || 
             comandoPost.value == 'iniciaappserver' || 
             comandoPost.value == 'reiniciaappserver' || 
             comandoPost.value == 'apagaappserver'
             ){
